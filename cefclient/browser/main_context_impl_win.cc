...
std::wstring MainContextImpl::GetAppWorkingDirectory() {
  ...
  //return szWorkingDir;
    //20230218
    wchar_t szWorkingDir[MAX_PATH + 1];
    if (_wgetcwd(szWorkingDir, MAX_PATH) == nullptr) {
        szWorkingDir[0] = 0;
    }
    else {
        // Add trailing path separator.
        size_t len = wcslen(szWorkingDir);
        szWorkingDir[len] = L'\\';
        szWorkingDir[len + 1] = 0;
    }
    return szWorkingDir;
