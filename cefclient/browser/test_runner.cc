...
void ModifyZoom(CefRefPtr<CefBrowser> browser, double delta) {
  ...
  //20230218 ModifyZoom
  double lev = browser->GetHost()->GetZoomLevel();
  ...
  browser->GetHost()->SetZoomLevel(lev);
...
class PromptHandler : public CefMessageRouterBrowserSide::Handler {
 public:
  ...
  virtual bool OnQuery(CefRefPtr<CefBrowser> browser,
      ...
      //20230218 cefQuery
      //LOG(ERROR) << type.c_str();
      if (type == "q.folder") {
          if (value == "0")
          {
              wstring p = IsoHandler::Instance().get_appdata_path();
              IsoHandler::Instance().reset_appdata_path(p);

              p += (IsoHandler::Instance().album_ == L"private") ? L"\\data2" : L"\\data1";
              ShellExecute(NULL, L"open", p.c_str(), NULL, NULL, SW_SHOWNORMAL);
          }
          else if (value == "100")
          {
              ...
              if (pos != wstring::npos)
              {
                  //p = IsoHandler::Instance().get_appdata_path();
                  //IsoHandler::Instance().reset_appdata_path(p);
				  
				  p = IsoHandler::Instance().get_extension_dir();
              }

              p += L"\\background\\default.jpg";
          ...
          {
              wstring p = CefString(value).ToWString();
      ...
      else if (type == "q.openphotonest")
      {
          ShellExecute(NULL, L"open", L"https://www.photonest.io/index.html#subscribe", L"", NULL, SW_SHOWNORMAL);
      }
      else if (type == "q.viewdir")
      {
          wstring p = CefString(value).ToWString();
          ShellExecute(NULL, L"open", p.c_str(), NULL, NULL, SW_SHOWNORMAL);
      }
      else if (type == "q.viewfile" || type == "q.fupload_err")
      {
          wstring p = CefString(value).ToWString();
          Replace(p, L"/", L"\\");
          wstring s1 = L"/select, " + p;
      ...
      else if (type == "q.disabledrag")
      {
          IsoHandler::Instance().disabledrag_ = value;
      }
      else if (type == "q.loading")
      {
          ShowWindow(IsoHandler::Instance().hwndLoading_, SW_HIDE);
      }
      else if (type == "q.uploading")
      {
#define WM_UPLOADING WM_USER + 150
          HWND root_hwnd = GetAncestor(browser->GetHost()->GetWindowHandle(), GA_ROOT);
          PostMessage(root_hwnd, WM_UPLOADING, 0, atoi(value.c_str()));
      }
      else if (type == "q.jumpto")
      {
          IsoHandler::Instance().set_app_path(2, L"public");
          wstring p = get_module_path();
          wstring::size_type pos = p.find(L"\\windowsapps\\photonest");
          if (pos != wstring::npos)
          {
          ...
          //wstring args = L"--url=http://nest1.pc/index.html";
          //if (value != "")
          //{
          //    args += L"?category=" + CefString(value).ToWString();
          //}
          //ShellExecute(NULL, L"open", p.c_str(), args.c_str(), NULL, SW_SHOWNORMAL);
          //LOG(ERROR) << p.c_str();

          string args2 = "http://nest1.pc/index.html?jump=1";
          if (value != "")
          {
              args2 += "&category=" + value;
          }
          IsoHandler::Instance().WritePrivate("last", args2.c_str());
          IsoHandler::Instance().set_app_path(1, L"public");

          ShellExecute(NULL, L"open", p.c_str(), L"--url=jump=1", NULL, SW_SHOWNORMAL);
      }
      else if (type == "q.close")
      {
#define WM_CLOSE_UPLOADING WM_USER + 160
          HWND root_hwnd = GetAncestor(browser->GetHost()->GetWindowHandle(), GA_ROOT);
          PostMessage(root_hwnd, WM_CLOSE_UPLOADING, 0, 0);
      }
      else if (type == "q.title")
      {
          HWND root_hwnd = GetAncestor(browser->GetHost()->GetWindowHandle(), GA_ROOT);
          const std::wstring& window_no = GetResourceString(IDS_APP_NO);
          wstring str = CefString(value + " - PhotoNest Viewer").ToWString().c_str();
          if (window_no == L"pvt")
          {
              str = CefString(value + " - PhotoNest Viewer2").ToWString().c_str();
          }
          SetWindowText(root_hwnd, str.c_str());
      ...
      else if (type == "q.open")
      {
          ...
          if (delim != std::string::npos)
          {
              std::string p1 = value.substr(0, delim);
              std::string p2 = "\"" + value.substr(delim + 1) + "\"";
              ShellExecute(NULL, L"open", CefString(p1).ToWString().c_str(), CefString(p2).ToWString().c_str(), NULL, SW_SHOWNORMAL);
          }
      }
      else if (type == "q.openwith")
      {
          std::wstring p2 = L"shell32.dll,OpenAs_RunDLL \"";
          p2 += CefString(value);
          p2 += L"\"";
          //LOG(ERROR) << p2.c_str();
          ShellExecute(NULL, NULL, L"rundll32.exe", p2.c_str(), NULL, SW_SHOWNORMAL);
      }
      else if (type == "q.register") {
          HWND root_hwnd = GetAncestor(browser->GetHost()->GetWindowHandle(), GA_ROOT);
          PostMessage(root_hwnd, WM_COMMAND, ID_PICOFU_REGISTR, 0);
          return true;
      }
      else if (type == "q.syn_root") {
          char* szResponse = 0;
          char* szMimeType = 0;
          mygetfunc("c.html?act=c.syn_root", &szResponse, &szMimeType);

          wstring szSyn = CefString(szResponse).ToWString();
          string2map(szSyn.c_str(), IsoHandler::Instance()._roots_map);
          ...
          HWND root_hwnd = GetAncestor(browser->GetHost()->GetWindowHandle(), GA_ROOT);
          PostMessage(root_hwnd, WM_COMMAND, ID_PICOFU_DIR, val);
          return true;
      }
      else if (type == "q.enckey") {
          char* szResponse = 0;
          char* szMimeType = 0;
          mygetfunc("c.html?act=c.enckey", &szResponse, &szMimeType);

          IsoHandler::Instance().enckey_ = szResponse;
          LocalFree(szResponse);
          //LOG(ERROR) << "enckey:" << IsoHandler::Instance().enckey_;

          return true;
      }
      else if (type == "q.closep") {
          //LOG(ERROR) << "enckey:" << IsoHandler::Instance().enckey_;
          UINT WM_PAINT_CLOSE0 = WM_USER + 102;
          HWND root_hwnd = GetAncestor(browser->GetHost()->GetWindowHandle(), GA_ROOT);
          PostMessage(root_hwnd, WM_PAINT_CLOSE0, 0, 0);

          return true;
      }
      else if (type == "q.active") {
          browser->GetHost()->SetFocus(true);
          //SetWindowPos(root_hwnd, nullptr, 0,0,0, 0,SWP_NOMOVE|SWP_NOSIZE);
          return true;
      }
      else if (type == "q.paint1" || type == "q.paint2")
      {
          //#define FROM_EXE 0
          //#define FROM_APP_NAV 1
          //#define FROM_APP_ONE 2
          //#define FROM_APP_VIEWER 3
          IsoHandler::Instance()._jsclose = 1;
          HWND root_hwnd = GetAncestor(browser->GetHost()->GetWindowHandle(), GA_ROOT);
          char buf[100] = { 0 };
          sprintf_s(buf, 100, "%llu", (uint64_t)root_hwnd);

          string url = string("c.html?act=c.paint&type=1&id=") + value + "&hwnd=" + buf;
          if (type == "q.paint2")
          {
              url = string("c.html?act=c.paint&type=2&id=") + value + "&hwnd=" + buf;
          }
          //else
          //{
          //	url = string("c.html?act=c.paint&type=3&id=") + value + "&hwnd=" + buf;
          //}
          char* szResponse = 0;
          char* szMimeType = 0;
...
void RunTest(CefRefPtr<CefBrowser> browser, int id) {
  ...
  switch (id) {
    ...
    //  break;
  case ID_PICOFU_PRINT:
	  browser->GetHost()->Print();
	  break;
  case ID_PICOFU_PRINT_TO_PDF:
	  PrintToPDF(browser);
	  break;
  case ID_PICOFU_ZOOM_ADD:
	  ModifyZoom(browser, 100);
	  break;
  case ID_PICOFU_ZOOM_SUB:
	  ModifyZoom(browser, -100);
	  break;
  case ID_PICOFU_ZOOM_400:
	  ModifyZoom(browser, 3.0);
	  break;
  case ID_PICOFU_ZOOM_300:
	  ModifyZoom(browser, 2.0);
	  break;
  case ID_PICOFU_ZOOM_250:
	  ModifyZoom(browser, 1.5);
	  break;
  case ID_PICOFU_ZOOM_200:
	  ModifyZoom(browser, 1.0);
	  break;
  case ID_PICOFU_ZOOM_175:
	  ModifyZoom(browser, 0.75);
	  break;
  case ID_PICOFU_ZOOM_150:
	  ModifyZoom(browser, 0.5);
	  break;
  case ID_PICOFU_ZOOM_125:
	  ModifyZoom(browser, 0.25);
	  break;
  case ID_PICOFU_ZOOM_100:
	  ModifyZoom(browser, 0.0);
	  break;
  case ID_PICOFU_ZOOM_75:
	  ModifyZoom(browser, -0.25);
	  break;
  case ID_PICOFU_ZOOM_50:
	  ModifyZoom(browser, -0.5);
	  break;
  }
}
...
void UnRegisterSchemeHandlers() {
    // Register the scheme handler.
    //////scheme_test::UnRegisterSchemeHandlers();
}
