...
bool ClientHandler::OnJSDialog(CefRefPtr<CefBrowser> browser,
    ...
    {
        ATOM atom = GlobalAddAtom(message_text.ToWString().c_str());
        HWND root_hwnd = GetAncestor(browser->GetHost()->GetWindowHandle(), GA_ROOT);
        PostMessage(root_hwnd, WM_COMMAND, IDD_DIALOG_ALERRT, (LPARAM)atom);
    ...
    else if (dialog_type == JSDIALOGTYPE_CONFIRM)
    {
        ATOM atom = GlobalAddAtom(message_text.ToWString().c_str());
        HWND root_hwnd = GetAncestor(browser->GetHost()->GetWindowHandle(), GA_ROOT);
        PostMessage(root_hwnd, WM_COMMAND, IDD_DIALOG_CONFIRM, (LPARAM)atom);
        suppress_message = false;
        return true;
    }
...
void ClientHandler::OnBeforeContextMenu(CefRefPtr<CefBrowser> browser,
  ...
  */
  //zhenxiufeng OnBeforeContextMenu
  //model->Clear();

  model->Remove(100); //back
  model->Remove(101); //forward
  model->Remove(-1);
  model->Remove(131); //print
  model->Remove(132); //view page source
  if ((params->GetTypeFlags() & (CM_TYPEFLAG_PAGE | CM_TYPEFLAG_FRAME)) != 0) {
      // Add a separator if the menu already has items.
     // if (model->GetCount() > 0)
      //  model->AddSeparator();
    ...
    //  if (!use_chrome_runtime) {
	  ...
	  {
		  model->AddItem(CLIENT_ID_SHOW_DEVTOOLS, "&Show DevTools");//20240716
	  }
     ...
     //  }
      ...
      {
          if (IsoHandler::Instance().flag_video_)
          {
              model->Remove(CLIENT_ID_SHOW_SSL_INFO);
          }
          else
          {
              wstring tips = L"";
              transl("Exit full screen", tips);
              model->AddItem(CLIENT_ID_SHOW_SSL_INFO, tips);
          }
      }
      else
      {
      ...
      // if (!use_chrome_runtime) {
         // TODO(chrome-runtime): Add support for this.
      //   model->AddSeparator();
      //   model->AddCheckItem(CLIENT_ID_CURSOR_CHANGE_DISABLED,                          "Cursor change disabled");
       //  if (mouse_cursor_change_disabled_)
       //    model->SetChecked(CLIENT_ID_CURSOR_CHANGE_DISABLED, true);
       //}
...
void ClientHandler::OnFullscreenModeChange(CefRefPtr<CefBrowser> browser,
  ...
  if (fullscreen)
  {
      IsoHandler::Instance().flag_video_ = true;
  }
  else
  {
      IsoHandler::Instance().flag_video_ = false;
  }
  //LOG(ERROR) << "OnFullscreenModeChange " << fullscreen;
...
bool ClientHandler::OnConsoleMessage(CefRefPtr<CefBrowser> browser,
                                     ...
                                     int line) {
  CEF_REQUIRE_UI_THREAD();
  if (!IsoHandler::Instance().debug_ && level != LOGSEVERITY_ERROR)
  {
	  return false;
  }

  FILE* file = _wfopen(console_log_file_.c_str(), L"a");
  if (file) {
    std::stringstream ss;
    ss << "Level: ";
    switch (level) {
...
bool ClientHandler::OnDragEnter(CefRefPtr<CefBrowser> browser,
    ...
    string::size_type pos = url.find("admin.html?page=admin.photos_add");
    if (pos != string::npos)
    {
        return false;
    }
    ...
    {
        pos = url.find(".pc/view.html");
        if (pos != string::npos)
        {
            if (IsoHandler::Instance().disabledrag_ == "0")
            {
                return false;
            }
        }
...
//20230218 OnPreKeyEvent
  ...
  if (event.type == KEYEVENT_RAWKEYDOWN && event.windows_key_code == VK_F11)
  {
      ...
      if (Style & DS_MODALFRAME)
      {
          if (IsoHandler::Instance().flag_video_)
          {
              string js = "if(typeof stopv === \"function\") stopv(); else  document.webkitExitFullscreen();";
              frame->ExecuteJavaScript(js, frame->GetURL(), 0);
              //frame->ExecuteJavaScript("{document.webkitCancelFullScreen()}", frame->GetURL(), 0);
          }
          else
          {
              NotifyFullscreen(false);
      ...
      else
      {
          ...
          //frame->ExecuteJavaScript("{var obj=document.getElementById(\"vdo\"); if(obj) obj.webkitRequestFullScreen();}", frame->GetURL(), 0);
              //frame->ExecuteJavaScript("{document.webkitCancelFullScreen()}", frame->GetURL(), 0);
      }
  }
  ...
  if (GetKeyState(VK_CONTROL) < 0 && event.type == KEYEVENT_RAWKEYDOWN && (event.windows_key_code == VK_ADD || event.windows_key_code == VK_OEM_PLUS))
  {
  ...
  else if (GetKeyState(VK_CONTROL) < 0 && event.type == KEYEVENT_RAWKEYDOWN && (event.windows_key_code == VK_SUBTRACT || event.windows_key_code == VK_OEM_MINUS))
  {
      double lev = browser->GetHost()->GetZoomLevel();
      if (lev == -0.5)
      {
  ...
  else if (GetKeyState(VK_CONTROL) < 0 && event.type == KEYEVENT_RAWKEYDOWN && (event.windows_key_code == VK_NUMPAD0 || event.windows_key_code == 0x30))
  {
...
void ClientHandler::NotifyFullscreen(bool fullscreen) {
  ...
  //LOG(ERROR) << "NotifyFullscreen " << fullscreen;
