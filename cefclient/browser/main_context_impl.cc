...
MainContextImpl::MainContextImpl(CefRefPtr<CefCommandLine> command_line,
  ...
  char* szResponse = 0;
  char* szMimeType = 0;
  mygetfunc("c.html?act=c.init", &szResponse, &szMimeType);
  ...
  {
	  type = "public";
	  string::size_type pos = main_url_.find("nest1.pc/index.html");
	  if (pos == string::npos)
	  {
		url2 = "http://nest1.pc/last.html";
	  }
  }
  else
  {
	  ...
	  if (main_url_.empty())
	  {	
		url2 = "http://nest2.pc/a.html";
	  }
  }

  string::size_type pos1 = main_url_.find(".pc/view.html");
  if (pos1 == string::npos)
  {
      main_url_ = url2;
  }
  ...
  if (type != "")
  {
	  string url = "c.html?act=c.alb&alb=";
	  url += type;

	  string::size_type pos = main_url_.find(".pc/view.html");
	  if (pos != string::npos)
	  {
		  url += "&viewer=1";
	  }
      ...
      wstring s = CefString(szResponse);
	  string2map(s.c_str(), IsoHandler::Instance()._roots_map);
...
std::wstring MainContextImpl::GetConsoleLogPath() {
    //return GetAppWorkingDirectory() + "console.log";

    std::wstring ret = GetAppWorkingDirectory() + L"console.log";
    ...
    if (SUCCEEDED(SHGetKnownFolderPath(FOLDERID_LocalAppData, 0, NULL, &szDocument)))
    {
        ...
        if (pos != wstring::npos)
        {
            ...
            else if (IsoHandler::Instance().viewer_ == 2)
            {
                ret += L"\\Nest1";
            }
            else
