    ...
    $.getJSON('./ws.php?method=upload.complete&id=' + catids + '&cancel=' + cancel + "&nb=" + uploadedPhotos.length, function (data) {
        ...
        if (uploadedPhotos.length > 0) {
            ...
            $("a.preview-box").colorbox({
                transition: "none", width: "85%", height: "85%", fixed: true, maxWidth: 1140
            });
        }
        else {
...
function pre_data() {
    $("a.openf").colorbox({
        transition: "none", width: 800, height: 495, fixed: true, iframe: true,
        onClosed: function () {
...
function remove_folder(rootid) {
    ...
    $.colorbox({
        transition: "none", width: 485, height: 300, fixed: true, iframe: true, overlayClose: false, opacity: 0.5, closeButton: true,
        title: _tit10,
