...
function scroll0() {
    ...
    //if (document.body.scrollHeight - document.body.clientHeight > h) 
    {
        if (sc > h) {
            if (!b) {
                header58();
...
function gopic(url) {
    ...
    _map = 1;
    $.colorbox({
        transition: "none", width: "100%", height: "100%", fixed: true, iframe: true,
        href: url,
        onComplete: function () {
...
function set_bg2(bg) {
    ...
    if (_skin == "dark") {
        $('.nav').css('border', '0');
        $('.return').attr('style', 'color:#bbb !important');
    }
...
function show_filter(catid) {
    ...
    $.colorbox({
        transition: "none", width: "600", height: "600", fixed: true, iframe: true, maxWidth: 1140,
        title: _searchTitle,
        href: function () {
...
function addcaddie() {
    ...
    $.get("./index.html?act=index.addcaddie", function (data, state) {
        if (data != " ") {
            $("#cmdCaddie").html(data);
        }
    });
...
function addto_fav(obj) {
    ...
    if ($(obj).hasClass('stm-icon-favorite-add')) {
        url = "./picture.html?act=picture.add_to_favorites&picid=" + ids;
    }

    $.get(url, function (data, state) {
        $(obj).toggleClass('stm-icon-favorite-add stm-icon-favorite-del');
...
function delabox() {
    ...
    $.colorbox({
        transition: "none", width: 437, height: 270, fixed: true, iframe: true, overlayClose: false,
        title: _tit20,
        href: function () {
            var nb = 0;
            var items = $("#mydiv").find('img');
...
function playbox(url) {
    $.colorbox({
        href: url, transition: "none", width: "95%", height: "95%", fixed: true, //maxWidth: 1140,
        onComplete: function () {
        },
        onClosed: function () {
        }
...
function mbprofile() {
    ...
    $.colorbox({
        ...
        transition: "none", width: width0, height: height0, iframe: true, fixed: true, maxWidth: 1140,
        title: _profiletitle
    });
}
...
function mbskins() {
    $.colorbox({
        ...
        transition: "none", width: "95%", height: "95%", iframe: true, fixed: true, maxWidth: 1140,
        title: _skintitle
    });
}
...
$(document).ready(function () {
    ...
    if (location.href.indexOf('favorites') >= 0) {
        $('.stm-icon-favorite2').toggleClass('stm-icon-favorite-add stm-icon-favorite-del');
    }
