...
function cleanup() {
    $.get("./view.html?his=view.clear", function (data, state) {
        $("#clean").empty();
...
function addto() {
    ...
    $.getJSON('./ws.php?method=viewer.addto&type=add&category=viewer&src=' + getSrc(), function (data) {
        window.cefQuery({
            request: 'Prompt.q.jumpto:' + data.category
        });
    });
}
...
$(document).ready(function () {
    ...
    $.fn.uploader = function (options) {
        return this.each(function (index) {
            ...
            var dom = {
                uploaderBox: $(this),
                selectButton: $('<input style="height: 0; width: 0;" accept=".jpg,.jpeg,.png,.gif,.webp,.bmp,.dib,.mp3,.ogg,.mp4,.webm;" id="fileinput' + index + '" type="file" >')
            };
