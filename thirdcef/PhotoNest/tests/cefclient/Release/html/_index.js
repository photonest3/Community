...
function switchSize(arg) {
    window.location = _switchBoxSizeUrl + "&" + arg;
}
function switchOrder(arg) {
    window.location = _switchBoxSortUrl + "&" + arg;
}
...
function gourl(url) {
    $.get(url, function (data, state) {
        ...
        $("#thumb-wap").html(o);
        if (url.indexOf('details') > 0 || url.indexOf('mlist') > 0 || url.indexOf('ylist') > 0 || url.indexOf('start') > 0 || url.indexOf('index.face') > 0) {
        ...
        if (url.indexOf('index.face') > 0) {
            $("#oooo").html("");
            $("#titleh2").html(info.title);
            $(".categoryActions").html(info.actions);

            $(".calendarViews").remove();
...
function gocat(catid) {
    window.location = "./index.html?category="+catid;
}
...
function show_face(catid) {
    $.colorbox({
        transition: "none", width: "500", height: "620", fixed: true, iframe: true, maxWidth: 1140, opacity: .3, overlayClose: true, right: 10,
        title: _faceTitle,
        href: function () {
            var url = "./index.html?act=index.face_box&category=" + catid;
            return url;
        },
        onComplete: function () {
			 //$('#colorbox').css('border-radius', '0');
			 $('#colorbox').css('box-shadow', 'unset');
			 //$('#colorbox').css('background', '#000');
...
function myreload() {
    ...
    if (ifm.length > 0) {
        var src = ifm[0].src;
        if (src.indexOf("/picture.html") >= 0 || src.indexOf("&osm=1") >= 0 || src.indexOf("index.film") >= 0 || src.indexOf("index.zoom") >= 0) {
            var pos = src.indexOf("&d=");
            if (pos > 0) {
                src = src.substring(0, pos);
            }
...
function switch2() {
    var SwitchBox = [];
    SwitchBox.push("#searchLink", "#searchBox");
    SwitchBox.push("#calendarViewSwitchLink", "#calendarViewSwitchBox");
    SwitchBox.push("#sortOrderLink", "#sortOrderBox");
    SwitchBox.push("#derivativeSwitchLink", "#derivativeSwitchBox");
